<div class="space-y-6">
        ...
        <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
            <div class="mb-5 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <span class="rounded-lg bg-blue-500 px-3 py-2 text-sm font-semibold text-white">{{ $calendar['monthLabel'] }}</span>
                    <span class="rounded-lg bg-blue-500 px-3 py-2 text-sm font-semibold text-white">{{ $calendar['year'] }}</span>
                </div>
            </div>

            <div class="grid grid-cols-7 gap-2 text-center text-sm text-slate-500">
                ...
                @foreach ($calendar['weeks'] as $week)
                    @foreach ($week as $day)
                        <div class="rounded-xl border px-2 py-3 font-medium {{ $day['isToday'] ? 'border-blue-500 bg-blue-50 text-blue-600' : ($day['isCurrentMonth'] ? 'border-slate-200 text-slate-700' : 'border-transparent bg-slate-50 text-slate-300') }}">
                            {{ $day['day'] }}
                        </div>
                    @endforeach
                @endforeach
    ...
    <div class="grid gap-6 2xl:grid-cols-[1.4fr_0.7fr]">
            ...
            <div class="space-y-4 px-6 py-5">
                @forelse ($agendaItems as $agenda)
                    <div class="flex gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-4">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-slate-900">{{ $agenda['dateLabel'] }}</p>
                            <p class="mt-1 text-sm text-slate-700">{{ $agenda['title'] }}</p>
                            <p class="mt-1 text-xs text-slate-400">{{ $agenda['subtitle'] }}</p>
                        </div>
                    </div>
                @empty
