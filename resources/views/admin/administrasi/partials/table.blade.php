    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-slate-200 text-sm text-slate-700">
            ...
            <tbody class="divide-y divide-slate-200 bg-white">
                    ...
                    <tr class="transition hover:bg-slate-50">
                        @if($canDelete)
                            <td class="px-4 py-4 align-top">
                                <input type="checkbox"
                                       value="{{ $item->id_dokumen_administrasi }}"
                                       class="row-checkbox-{{ $tableId }} h-5 w-5 rounded border-slate-300 text-blue-600 focus:ring-blue-500">
                            </td>
                        @endif
                        <td class="px-4 py-4 font-medium text-slate-900">{{ $item->id_dokumen_administrasi }}</td>
                        ...
                        <td class="px-4 py-4">
                            <div class="flex items-center justify-center gap-2">
                                @if($canEdit)
                                    <a href="{{ route('administrasi.edit', $item->id_dokumen_administrasi) }}"
                                ...
                                @if($canView)
                                    <a href="{{ route('administrasi.show', $item->id_dokumen_administrasi) }}"
                                ...
                                @if($canDelete)
                                    <form action="{{ route('administrasi.destroy', $item->id_dokumen_administrasi) }}"
                                          method="POST"
                ...
                @empty
                    <tr>
                        <td colspan="{{ ($selectedKategori ?? null) === 'Siswa' ? ($canDelete ? 9 : 8) : ($canDelete ? 7 : 6) }}" class="px-4 py-12 text-center text-sm text-slate-500">
                            {{ $emptyMessage }}
                        </td>
                    </tr>
                @endforelse
...
<script>
    (function () {
        ...
        const bulkDeleteForm = document.getElementById('bulkDeleteAdministrasiForm');
        const bulkDeleteButton = document.getElementById('bulkDeleteAdministrasiButton');
        ...
        if (bulkDeleteButton && bulkDeleteForm) {
            bulkDeleteButton.addEventListener('click', function () {
                ...
                if (selectedIds.length === 0) {
                    alert('Pilih minimal satu data administrasi untuk dihapus.');
                    return;
                }

                if (!confirm('Yakin ingin menghapus semua data administrasi yang dipilih?')) {
                    return;
                }
