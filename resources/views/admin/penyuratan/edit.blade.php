...
@section('content')
@php
    $defaultJenis = old('jenis_surat', strtolower($surat->jenis->nama_jenis_surat ?? 'masuk'));
@endphp
...
<div class="max-w-4xl space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Perbarui data surat dan ganti file bila memang diperlukan.</p>
        </div>
    </div>
    ...
    <form action="{{ route('penyuratan.update', $surat->id_dokumen_penyuratan) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid gap-5 md:grid-cols-2">
            ...
            <div class="md:col-span-2">
                ...
                <input id="nama_dokumen"
                       type="text"
                       name="nama_dokumen"
                       value="{{ old('nama_dokumen', $surat->nama_dokumen) }}"
                       placeholder="Nama"
                       class="w-full rounded-xl border border-blue-100 bg-white px-5 py-4 text-slate-800 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                       required>
            </div>
                    ...
                    @foreach(['masuk' => 'Masuk', 'keluar' => 'Keluar'] as $value => $label)
                        ...
                        <label class="cursor-pointer">
                            <input type="radio"
                                   ...
                                   {{ $checked ? 'checked' : '' }}
                                   required>
                            <span class="inline-flex min-w-[140px] items-center justify-center rounded-xl border border-slate-200 px-6 py-3 text-base font-semibold transition {{ $checked ? 'bg-blue-600 border-blue-600 text-white shadow-sm' : 'bg-white text-slate-700 hover:border-blue-300 hover:text-blue-600' }}">
                                {{ $label }}
                            </span>
                        </label>
                    @endforeach
            ...
            <div>
                ...
                <input id="nama_pengirim_penerima"
                       ...
                       value="{{ old('nama_pengirim_penerima', $surat->nama_pengirim_penerima) }}"
        ...
        <div>
            ...
            <label for="file_surat"
                ...
                <span class="mt-3 text-sm text-slate-500">File saat ini: {{ basename($surat->file_path) }}</span>
