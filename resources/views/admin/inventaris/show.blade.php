...
<div class="space-y-6">
    <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-4xl font-semibold text-slate-900">Preview Inventaris</h1>
                <p class="mt-1 text-sm text-slate-500">Halaman ini read-only untuk melihat detail dokumen inventaris.</p>
            </div>
        </div>

        <div class="flex flex-wrap items-center gap-3">
            <a href="{{ route('inventaris.edit', $inventaris->id_dokumen_inventaris) }}"
        ...
        <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="mt-6 space-y-5">
                <div>
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-800">{{ $inventaris->nama_dokumen }}</div>
                </div>

                <div>
                    <label class="mb-2 block text-sm font-medium text-slate-500">Tanggal</label>
                    <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-800">{{ \Illuminate\Support\Carbon::parse($inventaris->tanggal_dokumen)->format('d F Y') }}</div>
                </div>

                <div>
                    <label class="mb-2 block text-sm font-medium text-slate-500">Diupload Oleh</label>
                    <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-800">{{ $inventaris->created_by }}</div>
                </div>

                <div>
                    <label class="mb-2 block text-sm font-medium text-slate-500">Nama File</label>
                    <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-800">{{ basename($inventaris->file_path) }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                @if($fileUrl && $isPreviewable)
                    @if(\Illuminate\Support\Str::endsWith(strtolower($inventaris->file_path), ['.jpg', '.jpeg', '.png']))
                        <img src="{{ $fileUrl }}" alt="{{ $inventaris->nama_dokumen }}" class="h-[620px] w-full object-contain bg-white">
                    @else
